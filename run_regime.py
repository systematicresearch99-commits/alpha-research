...
Mirrors run.py in structure — same pipeline pattern, same data loading,
same backtest engine, same metrics. Just adds regime labeling on top.
...
-----
    ...
    python run_regime.py --strategy sma --ticker SPY --start 2018-01-01
...
# ── Strategy registry (same as run.py) ───────────────────────────────────────
STRATEGIES = {
...
# ── Defaults ─────────────────────────────────────────────────────────────────
DEFAULT_INDEX   = "SPY"       # benchmark for index_correlation feature
DEFAULT_MODEL   = "results/regime_model.pkl"
DEFAULT_WINDOW  = 20          # rolling feature window (trading days)
...
def run_regime_analysis(
    ...
    **strategy_kwargs : passed through to generate_signals()
    ...
    (df_labeled, regime_results)
      df_labeled     : backtest DataFrame with 'regime' column attached
      regime_results : dict with 'overall', 'by_regime', 'regime_summary', 'df_labeled'
    """
    ...
    # ── 2. Run the strategy backtest (unchanged — uses your engine.py) ────────
    ...
    # ── 6. Save to SQLite (reuses store.py — saves full-period run as normal) ─
    if save:
        ...
        run_id = save_run(
            ...
            trades_df  = trades_df,
            notes      = notes or f"Regime analysis run — HMM 4-state",
        )
...
def run_all_strategies_regime(
    ...
    **strategy_kwargs,
    ...
    A single shared model ensures regime labels are consistent across
    strategy comparisons (apples-to-apples).
    """
    print(f"\n{'='*60}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("  SPY 2015→2024 | SMA + RSI across Bull/Bear/Crisis/Transition")
        print("=" * 60)
